...
Gridmix is a Vue 2 static site generator and development server built around a Node.js bootstrap pipeline, a plugin API, a [LokiJS](https://github.com/techfort/LokiJS)-backed content store, generated GraphQL schema, generated Vue Router routes, and Webpack 5 client/server bundles. This document describes how the codebase works today.
...
Each content collection is a `gridmix/lib/store/Collection.js` instance backed by [a Loki collection](http://techfort.github.io/LokiJS/Collection.html). Collections track:
...
Transformers are selected by MIME type. Source plugins usually create nodes with internal content metadata of:
- `internal.mimeType`
- `internal.content`
- `internal.origin`
...
`@gridmix/source-filesystem` is the local-file source. It does:
- Collection setup:
...
`entry.client.js` is the browser entry responsible for:
...
`entry.server.js` is the SSR entry responsible for:
- Per-render setup:
  - Runs project main.
...
The compiler exposes [Tapable hooks](https://github.com/webpack/tapable):
